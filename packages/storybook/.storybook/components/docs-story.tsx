...
import { Anchor, Description, type DocsStoryProps, useOf } from '@storybook/blocks'
...
const removeEmptyEventHandlers = (code: string): string => {
    ...
    const hasNeedlessFragment =
        withoutEmptyHandlers.startsWith('<>') && withoutEmptyHandlers.endsWith('</>')
...
> = ({ __forceInitialArgs = false, __primary = false, of, showDescription, showTitle }) => {
