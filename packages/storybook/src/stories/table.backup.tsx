...
function Template({ rows, ...args }: ComponentProps<typeof Table<StockWatchlistItem>>) {
    ...
    const renderEmptyState: ComponentProps<typeof Table<StockWatchlistItem>>['renderEmptyState'] =
        useCallback(() => {
            return (
                <EmptyState
                    actions={[
    ...
    return (
        <section className='bg-base w-full px-6 py-4'>
            ...
            <div className='mt-4 ml-auto flex items-center justify-center gap-2'>
                {Array.from({
                    length: Math.ceil((data?.meta.total ?? 0) / (data?.meta.perPage ?? 0)),
                }).map((_, i) => (
                    <Button
                        isIcon
                        onPress={() => setPage(i)}
